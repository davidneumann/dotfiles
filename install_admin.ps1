...
#Fonts - Taken from https://www.alkanesolutions.co.uk/2021/12/06/installing-fonts-with-powershell/?doing_wp_cron=1693080005.5994629859924316406250
...
if (!(Test-Path $fontSourceFolder)) {
    ...
    #Loop through fonts in the same directory as the script and install/uninstall them
    foreach ($FontItem in (Get-ChildItem -Path $fontSourceFolder |
            Where-Object { ($_.Name -like '*.ttf') -or ($_.Name -like '*.otf') })) {
        Install-Font -fontFile $FontItem.FullName
    }
}
